...
command(skip,N1,N2) --> [skip], {N2 is N1+1}.
...
test :-
	data('parse',Parse),!,
	aeAnalysis(Parse,AEEntry,AEExit),!,
	write('Done, Parse = '),write(Parse),nl,nl,
	write('Done, Entry = '),write(AEEntry),nl,nl,
	write('Done, Exit = '),write(AEExit),nl,nl.
